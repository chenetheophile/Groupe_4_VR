...
public class ColorPickerTriangle : MonoBehaviour {
	...
	void Update () {
        if (tr != null)
        {
            if (selected)
            {
                Debug.Log("On select on prefab");
                if (HasIntersection(tr))
                {
    ...
    {
        tr = args.interactorObject.transform;
        selected = true;
    ...
    private bool HasIntersection(Transform origin)
    {
        MyPlane = new Plane(transform.TransformDirection(Vector3.forward), transform.position);
    ...
    private void SetColor()
    {
        ...
        TheColor = c;
        TheColor.a = 1f;
        FindObjectOfType<PaintBall>().color = TheColor;
        FindObjectOfType<PaintBrush>().color_brush = TheColor;
    }
